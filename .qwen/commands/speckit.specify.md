...
2. **在创建新分支之前检查现有分支**：
   ...
   c. 确定下一个可用编号：
      ...
      - 找到最高编号 N
      - 使用 N+1 作为新分支编号
   d. 使用计算的编号和短名称运行脚本 `.specify/scripts/powershell/create-new-feature.ps1 -Json "{{args}}"`：
      - 传递 `--number N+1` 和 `--short-name "your-short-name"` 以及功能描述
      - Bash 示例：`.specify/scripts/powershell/create-new-feature.ps1 -Json "{{args}}" --json --number 5 --short-name "user-auth" "Add user authentication"`
      - PowerShell 示例：`.specify/scripts/powershell/create-new-feature.ps1 -Json "{{args}}" -Json -Number 5 -ShortName "user-auth" "Add user authentication"`
   **重要提示**：
...
3.5. **产品文件管理（基于存量增量开发）**：
   a. **读取或创建产品文件**：
      - 检查是否存在 `.specify.specify/memory/product.md` 文件
      - 如果不存在，从 `.specify.specify/templates/product-template.md` 创建初始产品文件
      - 如果存在，读取现有产品文件内容
   b. **解析产品文件内容**：
      - 提取产品定位信息
      - 提取现有利益相关者列表
...
4. 遵循此执行流程：
    ...
    7. 识别关键实体（如果涉及数据）
    8. 从用户故事中识别产品特性：
       - 分析每个用户故事，识别其所属的产品特性领域
       - 将用户故事归类到现有特性或识别新特性
       - 为每个特性确定其价值主张和关联的利益相关者
    9. 更新产品文件：
       a. **更新产品定位**（如需要）：
          - 如果当前规范引入了新的产品定位或改变了现有定位，更新产品定位章节
          - 在版本变更历史中记录定位变更
       b. **更新利益相关者**（如需要）：
          - 检查规范中识别的新利益相关者是否已存在于产品文件中
          - 如果不存在，添加到利益相关者列表
          - 更新现有利益相关者的描述（如果规范提供了新信息）
          - 在版本变更历史中记录利益相关者变更
       c. **更新产品特性**：
          - 对于每个从用户故事识别出的特性：
            * 如果特性已存在：更新特性描述，添加新的关联用户故事
            * 如果特性不存在：创建新特性条目，包含描述、价值、关联利益相关者、关联用户故事
          - 确保每个用户故事都关联到至少一个特性
          - 更新特性的"最后更新"日期
          - 在版本变更历史中记录特性变更
       d. **更新版本信息**：
          - 根据变更类型更新产品版本号（遵循语义版本控制）
          - 更新"最后更新"日期为当前日期
          - 在版本变更历史表中添加新条目，记录本次变更
    10. 返回：成功（规范已准备好进行规划，产品文件已更新）
5. 使用模板结构将规范写入 SPEC_FILE，用从功能描述（参数）派生的具体细节替换占位符，同时保留章节顺序和标题。
   **重要**：在规范文件中添加一个章节，记录本规范关联的产品特性：
...
7. **产品文件更新报告**：
   - 列出对产品文件的变更：
     * 新增/修改的产品定位（如有）
     * 新增/修改的利益相关者（如有）
     * 新增/修改的产品特性（列出所有变更的特性）
     * 版本变更：旧版本 → 新版本
   - 确保产品文件反映了完整的全量信息（所有特性、所有利益相关者、当前产品定位）
8. 报告完成情况，包括：
   - 分支名称
   - 规范文件路径
   - 检查清单结果
   - 产品文件更新摘要
   - 下一阶段的就绪性（`/speckit.clarify` 或 `/speckit.plan`）
**注意**：
- 脚本在写入之前创建并检出新分支并初始化规范文件。
- 产品文件（`.specify.specify/memory/product.md`）是全局的、全量的产品定义，每次规范更新都会增量更新它。
- 每个规范文件只包含该规范的增量信息，而产品文件包含所有特性的完整视图。
## 一般指南
## 快速指南
- 专注于用户需要的**什么**和**为什么**。
