...
1. **设置**：从仓库根目录运行 `.specify/scripts/powershell/setup-plan.ps1 -Json` 并解析 JSON 以获取 FEATURE_SPEC、IMPL_PLAN、SPECS_DIR、BRANCH。对于参数中的单引号，如 "I'm Groot"，使用转义语法：例如 'I'\''m Groot'（如果可能，使用双引号："I'm Groot"）。

2. **加载上下文**：
   - 读取 FEATURE_SPEC 和 `.specify/memory/constitution.md`
   - **读取产品文档**：检查并加载 `.specify/memory/product.md`（如果存在）
     - 从产品文档中提取：
       - 产品定位和愿景（用于技术决策的上下文）
       - 利益相关方信息（用于理解需求来源）
       - 当前功能的关联产品特性信息（特性ID、业务价值、优先级）
   - **读取全局架构文档**（增量开发支持）：
     - **组件架构文档**：检查并加载 `.specify/memory/components-global.md`（如果存在）
       - 从组件文档中提取：
         - 现有组件列表、职责和边界
         - 组件类型和技术栈
         - 组件间的依赖关系
         - 组件的数据所有权
     - **数据模型文档**：检查并加载 `.specify/memory/data-model-global.md`（如果存在）
       - 从数据模型文档中提取：
         - 现有数据模型实体
         - 实体与组件的关联
         - 跨组件实体关系
     - **API 架构文档**：检查并加载 `.specify/memory/api-global.md`（如果存在）
       - 从 API 文档中提取：
         - 现有 API 端点
         - 端点与组件的关联
         - 组件间 API 调用关系
...
3. **更新全局数据模型** → `.specify/memory/data-model-global.md`（增量更新）：
   - **检查全局文件是否存在**：
     - 如果不存在：从模板创建 `.specify/memory/data-model-global.md`
       - 模板位置：`.specify.specify/templates/data-model-global-template.md` 或 `.specify/templates/data-model-global-template.md`
     - 如果存在：读取现有内容
...
5. **更新全局 API 架构** → `.specify/memory/api-global.md`（增量更新）：
   - **检查全局文件是否存在**：
     - 如果不存在：从模板创建 `.specify/memory/api-global.md`
       - 模板位置：`.specify.specify/templates/api-global-template.md` 或 `.specify/templates/api-global-template.md`
     - 如果存在：读取现有内容
...
6. **代理上下文更新**：
   - 运行 `.specify/scripts/powershell/update-agent-context.ps1 -AgentType qwen`
   - 这些脚本检测正在使用的AI代理
   - 更新相应的代理特定上下文文件
   - 仅添加来自当前计划的新技术
   - 保留标记之间的手动添加
...
- 全局级别：.specify/memory/data-model-global.md（更新）、.specify/memory/api-global.md（更新）
- 代理特定文件
