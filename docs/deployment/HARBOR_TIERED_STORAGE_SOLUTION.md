...
**重要说明:**
> ⚠️ Harbor Registry **不支持自动数据分层**。本方案提供 SSD 和 HDD 存储类，但需要**手动切换**使用哪个 PVC。自动分层功能需要 Harbor 原生支持或自定义开发。
...
```
┌─────────────────────────────────────────────────────────────┐
│  SSD (50Gi): harbor-registry-hot                            │
│  用途：热数据镜像 (最近访问)                                 │
│  状态：✅ 已创建，待使用                                     │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  HDD (2Ti): harbor-registry-warm ✅ 已使用                   │
│  用途：主存储 (所有镜像)                                     │
│  状态：✅ Harbor 正在使用                                    │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  HDD (8Ti): harbor-registry-cold                            │
│  用途：冷数据归档 (未来扩展)                                 │
│  状态：✅ 已创建，待使用                                     │
└─────────────────────────────────────────────────────────────┘
```
...
| 指标 | 实施前 | 实施后 | 提升 |
|------|--------|--------|------|
| **可用存储** | 50Gi (旧 PVC) | 2Ti (HDD) | **40 倍** |
| **存储成本** | $0.20/GB (SSD) | $0.05/GB (HDD) | **节省 75%** |
| **可存储镜像** | ~6 个 8GB | ~250 个 8GB | **40 倍** |
