...
- [ ] 测试覆盖 — 构造/`to_dict()`/HTTP 映射测试 + 编码唯一性测试通过（详见下方「异常测试用例规范」）
- [ ] BDD 验收场景 — 异常路径的 Gherkin 场景（如 `Scenario: 资源不存在返回 404`）

<details>
<summary><b>📋 异常测试用例规范（点击展开）</b></summary>

新增领域异常 `NewError`（假设编码 `EXCEPTION_NNN`，继承 `ParentError`，预期 HTTP 状态码 `NXX`）时，必须编写以下 5 组测试：

---

**1. 单元测试 — 构造与属性**（`tests/unit/domain/exceptions/test_[module].py`）

```python
class TestNewErrorInit:
    """NewError 初始化测试"""

    def test_default_message(self):
        """默认消息应为类级别 message"""
        ex = NewError()
        assert ex.message == "Expected default message"

    def test_custom_message(self):
        """自定义消息应覆盖默认值"""
        ex = NewError(message="custom msg")
        assert ex.message == "custom msg"

    def test_code_is_correct(self):
        """编码应为 EXCEPTION_NNN"""
        ex = NewError()
        assert ex.code == "EXCEPTION_NNN"

    def test_context_stored(self):
        """context 应被正确存储"""
        ex = NewError(context={"key": "val"})
        assert ex.context == {"key": "val"}

    def test_inherits_from_parent(self):
        """应继承自 ParentError"""
        assert issubclass(NewError, ParentError)

    # 如果异常有自定义构造器参数（如 transfer_id, role_id）
    def test_custom_constructor_params(self):
        """自定义构造器参数应正确设置"""
        ex = NewError(resource_id="abc-123")
        assert ex.resource_id == "abc-123"
        assert "abc-123" in ex.message
```

---

**2. 单元测试 — to_dict() 序列化**（同上文件）

```python
class TestNewErrorToDict:
    """NewError.to_dict() 序列化测试"""

    def test_basic_dict(self):
        """无 cause 时应返回 code/message/context"""
        ex = NewError(message="test", context={"k": "v"})
        d = ex.to_dict()
        assert d["code"] == "EXCEPTION_NNN"
        assert d["message"] == "test"
        assert d["context"] == {"k": "v"}

    def test_with_domain_cause(self):
        """cause 为领域异常时应递归序列化"""
        inner = ValidationError("inner error")
        ex = NewError(message="outer", cause=inner)
        d = ex.to_dict()
        assert d["cause"]["code"] == "EXCEPTION_201"
        assert d["cause"]["message"] == "inner error"

    def test_with_plain_cause(self):
        """cause 为普通异常时应序列化为 type/message"""
        inner = ValueError("raw error")
        ex = NewError(message="wrapped", cause=inner)
        d = ex.to_dict()
        assert d["cause"]["type"] == "ValueError"
        assert d["cause"]["message"] == "raw error"
```

---

**3. HTTP 映射测试**（`tests/unit/interfaces/api/test_exception_handlers.py`）

需要同时覆盖映射表正确性和端到端 HTTP 响应：

```python
class TestNewErrorHttpMapping:
    """NewError HTTP 映射测试"""

    # 3a. 映射表包含新类型
    def test_new_error_in_exception_http_map(self):
        """验证 EXCEPTION_HTTP_MAP 包含 NewError"""
        from src.interfaces.api.exception_handlers import EXCEPTION_HTTP_MAP
        assert NewError in EXCEPTION_HTTP_MAP

    def test_new_error_maps_to_correct_status(self):
        """验证 NewError 映射到预期 HTTP 状态码"""
        assert EXCEPTION_HTTP_MAP[NewError] == status.HTTP_NXX_XXX

    # 3b. _get_http_status 正确查找
    def test_get_http_status_for_new_error(self):
        """验证 _get_http_status 返回正确状态码"""
        from src.interfaces.api.exception_handlers import _get_http_status
        ex = NewError("test")
        assert _get_http_status(ex) == status.HTTP_NXX_XXX

    # 3c. 端到端 HTTP 响应（通过 TestClient）
    def test_new_error_http_response(self):
        """验证 NewError → JSON 响应结构完整"""
        app = FastAPI()
        register_exception_handlers(app)

        @app.get("/test")
        def route():
            raise NewError("test message", context={"detail": "info"})

        client = TestClient(app, raise_server_exceptions=False)
        resp = client.get("/test")

        assert resp.status_code == status.HTTP_NXX_XXX
        body = resp.json()
        assert body["error"]["code"] == "EXCEPTION_NNN"
        assert body["error"]["message"] == "test message"
        assert body["error"]["context"] == {"detail": "info"}
        assert "request_id" in body
        assert resp.headers["X-Error-Code"] == "EXCEPTION_NNN"
        assert "X-Request-ID" in resp.headers
```

---

**4. 编码唯一性测试**（`tests/unit/domain/exceptions/test_error_code_uniqueness.py`）

无需新增测试用例 — 现有 `test_all_error_codes_unique` 通过 `src.domain.exceptions.__all__` 反射扫描，新异常只要正确注册到 `__all__` 即自动覆盖。只需确保：
- [ ] 新异常在模块 `__all__` 中声明
- [ ] 新异常在包 `__init__.py` 的 `__all__` 中声明
- [ ] 运行 `pytest tests/unit/domain/exceptions/test_error_code_uniqueness.py -v` 通过

---

**5. BDD 验收场景**（`tests/acceptance/test_acceptance_[feature name].feature`）

对应每个需要向 API 消费者暴露的异常路径：

```gherkin
Scenario: 资源不存在时返回 404
  Given 系统中不存在 ID 为 "non-existent-id" 的资源
  When 客户端请求 GET /api/v1/resource/non-existent-id
  Then 响应状态码为 404
  And 响应体包含 "error"."code" 为 "EXCEPTION_NNN"
  And 响应体包含 "error"."message"
  And 响应体包含 "request_id"
  And 响应头 X-Error-Code 为 "EXCEPTION_NNN"

Scenario: 资源冲突时返回 409
  Given 系统中已存在名称为 "duplicate" 的资源
  When 客户端尝试创建同名资源
  Then 响应状态码为 409
  And 响应体包含 "error"."code" 为 "EXCEPTION_NNN"
```

BDD 步骤实现（`test_acceptance_[feature name].py`）参考模式：
```python
from pytest_bdd import given, when, then, parsers

@given("系统中不存在 ID 为 \"non-existent-id\" 的资源")
def resource_does_not_exist():
    pass  # 或 setup mock/stub

@when("客户端请求 GET /api/v1/resource/non-existent-id")
def client_requests_resource(test_client):
    # test_client 是 fixture 注入的 FastAPI TestClient
    # 使用 event_loop.run_until_complete() 而非 @pytest.mark.asyncio
    ...

@then(parsers.parse("响应状态码为 {status_code:d}"))
def response_status_is(status_code, test_client):
    assert test_client.last_response.status_code == status_code
```

> ⚠️ **BDD 步骤约束：** 步骤函数使用 `event_loop.run_until_complete()` 运行 async 调用，**禁止使用 `@pytest.mark.asyncio`**（会导致 context 数据丢失）。
...
```bash
# 单元测试
poetry run pytest tests/unit/domain/exceptions/test_[module].py -v
# HTTP 映射测试
poetry run pytest tests/unit/interfaces/api/test_exception_handlers.py -v -k "NewError"
# 编码唯一性
poetry run pytest tests/unit/domain/exceptions/test_error_code_uniqueness.py -v
# BDD 验收（如适用）
poetry run pytest tests/acceptance/test_acceptance_[feature name].py -v
```
