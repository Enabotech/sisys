...
|---------|---------------------|---------|
| 路由优先级 | 本地优先 80%，云端兜底 | 写成"云端优先" |
| 路由方式 | UDMR 三层动态决策（L1 合规→L2 评估→L3 执行） | 写成"静态路由" |
| 路由延迟 | P95<50ms（系统公理一硬约束） | 写成 P95<100ms |
| 成本目标 | 成本优化 50%、Token 成本节省 ≥50%（V1） | 缺失成本维度 |
| 本地模型路由占比 | MVP ≥60% / V1 ≥80% / V2 ≥85% | 引用错误版本值 |
...
**案例：** Story 1.17 中写成"云端优先"，architecture.md 明确为"本地优先 80%，云端兜底"；
...
- [ ] 路由优先级为"本地优先 80%，云端兜底"（非"云端优先"）
- [ ] 路由方式为"UDMR 三层动态决策"（非"静态路由"）
- [ ] 路由延迟引用正确的版本阈值（公理一硬约束 P95<50ms）
- [ ] 成本目标引用正确（Token 成本节省 ≥30% MVP / ≥50% V1 / ≥60% V2）
- [ ] 本地模型路由占比引用正确（≥60% MVP / ≥80% V1 / ≥85% V2）
...
| UDMR 框架 | 4 | 本地优先 80%、三层决策、成本优化 50% |
