...
│  │  │ ┌────────────────────────────────────────────────────────┐│  │  │
│  │  │ │ L1: TOOLS.md (< 200 tokens) - 工具元数据清单            ││  │  │
...
│  │  五层存储 / LLM Adapter / MCP (V2+) / 沙箱 / 消息总线实现         │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
#### 3.3.5 Checkpoint 管理（sisys checkpoint）
...
class ToolChainExecutor:
    ...
    async def execute_chain(self, chain_id: str, input_data: Dict) -> Dict:
        ...
        # 2. 有效性校验（检测循环依赖）
        if not dag.is_valid():
            raise ValueError("Invalid DAG: circular dependency detected")
