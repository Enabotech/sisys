...
|------|---------|---------|----------|
| **数据处理** | 17种格式解析、OCR、DQI、混合检索、知识图谱 | ❌ 未实现 | Epic 2-3 |
...
**全流程：** 数据接入（17种格式 + 断点续传）→ 解析提取（OCR/版面保留/表格语义）→ 质量治理（DQI 综合评分）→ 向量化检索（BGE-M3 三路召回 + RRF 融合 + ColBERT 重排序）→ 知识图谱（LLM + 规则混合实体抽取）→ 归档存储（WORM 7年）。
