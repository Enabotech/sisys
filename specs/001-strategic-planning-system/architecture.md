...
from src.infrastructure.agent_orchestration.langgraph_engine import LangGraphEngine

class StrategicOrchestrationService:
    """战略编排服务 - 协调Prefect工作流和LangGraph状态机"""
    
    @inject
    def __init__(
        self,
        prefect_engine: PrefectEngine = Provide["infrastructure.prefect_engine"],
        langgraph_engine: LangGraphEngine = Provide["infrastructure.langgraph_engine"],
        event_bus: EventBus = Provide["infrastructure.event_bus"]
    ):
    ...
    async def execute_document_processing(
        ...
        """
        执行文档处理编排
        流程：Prefect工作流处理 → 事件触发 → LangGraph分析
        """
        # 1. 启动Prefect文档处理工作流
        flow_run = await self.prefect_engine.execute_flow(
            flow_name="document-processing-flow",
            parameters={"documents": documents}
        )
        
        # 2. 监听工作流完成事件
        async def on_flow_complete(event_data):
            # 3. 提取处理结果
            processed_data = event_data["result"]
            
            # 4. 启动LangGraph Agent协作分析
            analysis_result = await self.langgraph_engine.execute_graph(
                graph_name="initial-analysis-graph",
                state={"processed_data": processed_data}
            )
            
            # 5. 如果有回调URL，发送结果
            if callback_url:
                await self._send_callback(callback_url, analysis_result)
                
            return analysis_result
            
        # 订阅Prefect工作流完成事件
        await self.event_bus.subscribe(
            event_type="prefect.flow_completed",
            handler=on_flow_complete,
            filter={"flow_id": flow_run.id}
        )
        
        return {"flow_id": flow_run.id, "status": "processing"}
    
    async def execute_strategic_planning(
        self,
        analysis_results: Dict[str, Any],
        checkpoint_handler: Optional[Callable] = None
    ) -> Dict[str, Any]:
        """
        执行战略规划编排
        流程：LangGraph BLM状态机 → 检查点 → Prefect报告生成
        """
        # 1. 初始化LangGraph规划状态机
        planning_state = await self.langgraph_engine.initialize_state(
            graph_name="blm-planning-graph",
            initial_state={
        ...
        for stage in stages:
            # 3. 执行当前阶段
            stage_result = await self.langgraph_engine.execute_stage(
                graph_name="blm-planning-graph",
                stage=stage,
            ...
            # 4. 检查点机制
            if stage_result.get("checkpoint_required", False):
                checkpoint_data = stage_result.get("checkpoint_data", {})
                
                # 调用用户检查点处理器
                if checkpoint_handler:
                    user_feedback = await checkpoint_handler(checkpoint_data)
                    stage_result["user_feedback"] = user_feedback
                    
                    # 应用用户反馈，继续执行
                    planning_state = await self.langgraph_engine.apply_feedback(
                        graph_name="blm-planning-graph",
                        state=planning_state,
            ...
            planning_state = stage_result.get("next_state", planning_state)
        
        # 5. 完成规划后，启动Prefect报告生成工作流
        report_flow = await self.prefect_engine.execute_flow(
            flow_name="report-generation-flow",
            parameters={
...
class RabbitMQEventBus:
    ...
    async def _setup_infrastructure(self):
        ...
        dlx_exchange = await self.channel.declare_exchange(
            name="strategy.events.dlx",
            type="direct",
            durable=True
        )
        
        dlx_queue = await self.channel.declare_queue(
            name="strategy.events.dlq",
            durable=True,
            arguments={
                "x-dead-letter-exchange": "strategy.events",
    ...
    async def publish(self, event: DomainEvent):
        ...
        # 记录发布日志
        await self._log_event_published(event)
    ...
    async def subscribe(
        ...
        """订阅事件"""
        if not queue_name:
            queue_name = f"strategy.{event_type}.queue"
        ...
        queue = await self.channel.declare_queue(
            ...
            durable=True,
            arguments={
                "x-dead-letter-exchange": "strategy.events.dlx",
        ...
        async with message.process():
            try:
                ...
                if event_type in self.handlers:
                    handler_info = self.handlers[event_type]
                    
                    # 应用过滤器（如果存在）
                    if handler_info["filter"]:
                        if not handler_info["filter"](event):
                    ...
                    try:
                        ...
                        await self._log_handler_error(event, e)
                        
            except Exception as e:
                # 消息格式错误，拒绝并不重新入队
                await message.reject(requeue=False)
                await self._log_message_error(message, e)
```

### **3. Prefect与LangGraph集成模式**

```python
# src/infrastructure/workflow/flows/integrated_planning_flow.py
...
from src.infrastructure.agent_orchestration.langgraph_engine import LangGraphEngine
from src.infrastructure.messaging.event_bus import RabbitMQEventBus

@task(name="preprocess-documents", retries=3, retry_delay_seconds=30)
async def preprocess_documents_task(documents: List[Dict]) -> Dict[str, Any]:
    """文档预处理任务"""
    # 使用Unstructured.io解析文档
    parsed_docs = []
    for doc in documents:
        parsed = await parse_document(doc["path"], doc["format"])
        parsed_docs.append(parsed)
    
    return {
        "parsed_documents": parsed_docs,
        "total_pages": sum(doc.get("page_count", 1) for doc in parsed_docs)
    }

@task(name="execute-agent-analysis", timeout_seconds=300)
async def execute_agent_analysis_task(processed_data: Dict[str, Any]) -> Dict[str, Any]:
    """执行Agent分析任务（调用LangGraph）"""
    ...
    initial_state = {
        ...
        "agents": ["ceo", "cfo", "cmo", "cto", "cho", "coo"],
        "current_tool": None,
        "analysis_results": {},
        "checkpoints": []
    }
    ...
    # 执行协作分析图
    result = await langgraph_engine.execute_graph(
        graph_name="collaboration-analysis-graph",
        state=initial_state,
        config={
            "max_iterations": 10,
            "temperature": 0.7,
    ...
    return {
        ...
        "checkpoints_reached": result.get("checkpoints", []),
        "total_iterations": result.get("iterations", 0)
    }

@task(name="generate-strategic-plan", retries=2)
async def generate_strategic_plan_task(analysis_results: Dict[str, Any]) -> Dict[str, Any]:
    """生成战略计划任务（协调Prefect和LangGraph）"""
    ...
    # 定义检查点处理器
    async def checkpoint_handler(checkpoint_data: Dict[str, Any]) -> Dict[str, Any]:
        """处理检查点事件"""
        # 发布检查点事件
        event = DomainEvent(
    ...
    # 执行战略规划编排
    from src.application.services.orchestration_service import StrategicOrchestrationService
    
    orchestration_service = StrategicOrchestrationService()
    
    planning_result = await orchestration_service.execute_strategic_planning(
        analysis_results=analysis_results,
...
@task(name="generate-pdf-report")
async def generate_pdf_report_task(planning_result: Dict[str, Any]) -> str:
    """生成PDF报告任务"""
    # 使用WeasyPrint或ReportLab生成PDF
    report_path = await generate_strategic_report(
...
@flow(
    name="integrated-strategic-planning-flow",
    description="集成的战略规划工作流（协调Prefect和LangGraph）",
    timeout_seconds=7200,  # 2小时超时
    retries=1
)
async def integrated_strategic_planning_flow(
    ...
    processed_data = await preprocess_documents_task(documents)
    
    # 阶段2：Agent协作分析（调用LangGraph）
    logger.info("开始Agent协作分析")
    analysis_results = await execute_agent_analysis_task(processed_data)
    
    # 阶段3：战略规划生成（协调Prefect和LangGraph）
    logger.info("开始战略规划生成")
    planning_result = await generate_strategic_plan_task(analysis_results)
    
    # 阶段4：报告生成（Prefect任务）
    logger.info("生成最终报告")
    report_path = await generate_pdf_report_task(planning_result)
    
    # 阶段5：清理和归档
    logger.info("归档结果到战略档案库")
    await archive_results(
        user_id=user_id,
    ...
    return {
        "status": "completed",
        "report_path": report_path,
        "planning_id": planning_result.get("planning_id"),
        "total_time": get_run_context().flow_run.total_run_time
    }
```
