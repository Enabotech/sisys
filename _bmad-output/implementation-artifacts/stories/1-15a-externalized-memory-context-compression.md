...
- [ ] MemoryMetadataRepository L2 存储（`src/infrastructure/storage/postgresql/repository/memory_metadata_repository.py`）
- [ ] MemoryChangeHistoryRepository L2 历史记录（`src/infrastructure/storage/postgresql/repository/memory_change_history_repository.py`）
- [ ] L0 写入成功率 100%（同步，强一致）
- [ ] L0 索引更新成功率 100%（同步）
...
- [ ] MemoryMetadata 实体（`src/domain/entities/memory_metadata.py`）
  - 字段: memory_id, user_id, name, description, type, path, version, mtime, owner, group_id, created_at, updated_at
...
- [ ] MemoryChangeHistory 实体（`src/domain/entities/memory_change_history.py`）
  - 字段: id (UUID), memory_id (UUID, 外键引用 MemoryMetadata.memory_id), version, changed_at, changed_by, change_type, changed_fields (JSONB), diff_summary, archived_ref
  ...
  - **memory_id**: UUID 外键引用 MemoryMetadata.memory_id（稳定引用，非按值外键）
- [ ] MemoryService 服务类（`src/domain/services/memory_service.py`）
  ...
  - **依赖倒置**: 通过 TextExtractorService 和 CompressorService 注入压缩逻辑，不直接依赖 application 层
  - **事件发布**: 通过 EventPublisherProtocol 注入（如 `EventPublisherProtocol | None = None`）

#### L1 压缩处理 (L1 Compression)
- [ ] **协议定义（领域层）**:
  - TextExtractorService（`src/domain/services/text_extractor_service.py`）- 文本提取接口
  - CompressorService（`src/domain/services/compressor_service.py`）- 压缩接口
  - 使用 Protocol 实现依赖倒置（复用 TriggerService 模式）
- [ ] L1TextExtractor 文本提取器（`src/application/text_processing/l1_text_extractor.py`）
  - 实现 TextExtractorService
...
- [ ] MemoryMetadataRepository L2 PostgreSQL 仓储（`src/infrastructure/storage/postgresql/repository/memory_metadata_repository.py`）
- [ ] MemoryChangeHistoryRepository L2 历史记录仓储（`src/infrastructure/storage/postgresql/repository/memory_change_history_repository.py`）
...
- [ ] 功能测试文件：`tests/acceptance/test_acceptance_externalized_memory_context_compression.feature`（由 Dev agent 在 Task 0 创建）
- [ ] 覆盖场景:
...
| **TDD 验收测试** | Gherkin 场景 | 业务价值验收 | `test_acceptance_externalized_memory_context_compression.feature` | Task 0 |
...
- [x] Subtask 0.11: 编写 Gherkin 验收测试 `tests/acceptance/test_acceptance_externalized_memory_context_compression.feature`（Dev agent 创建）
...
│   └── acceptance/
│       ├── test_acceptance_externalized_memory_context_compression.feature
...
- `src/domain/services/text_extractor_service.py` - TextExtractorService（文本提取接口，用于依赖倒置）
- `src/domain/services/compressor_service.py` - CompressorService（压缩接口，用于依赖倒置）
...
- `tests/acceptance/test_acceptance_externalized_memory_context_compression.feature` - Gherkin 验收测试（由 Dev agent 在 Task 0 创建）
...
- `src/domain/services/__init__.py` - 添加 MemoryService, TextExtractorService, CompressorService 导出
...
**更新说明:**
- v2.10.0: 架构 §11.2.5 DDL 同步更新：memory_metadata 添加 user_id/owner/group_id，path 格式改为 '{type}/{memory_id}.md'；memory_change_history.memory_name → memory_id (UUID)；同步更新实体表字段和 L1 触发流程接口
...
- v2.7.0: 修复 P1 架构一致性问题：(1) L0 文件命名采用方案 C（类型文件夹 + UUID：`{type}/{uuid}.md`）；(2) MEMORY.md 索引格式统一为 Markdown 链接格式 `- [name](type/uuid.md) — hook`（符合架构 §11.2.3）；(3) 多租户隔离方案设计（private/group 路径在 1.15b 实现）
