...
| AutoTriggerListener | Triggered | 1.14a | ✅ 已实现 |
| AutoRouteListener | Routed | 1.14b | ✅ 已实现 |
...
**Given** 应用启动
**When** 初始化事件监听器时
**Then** 通过 EventListenerRegistry 管理所有事件监听器：
  - MemoryChangedListener → memory.changed 队列
  - AutoTriggerListener → trigger.triggered 队列
  - AutoRouteListener → route.routed 队列
  - AutoExecuteCompletedListener → execute.completed 队列

**验证标准/Validation Criteria:**
- [ ] EventListenerRegistry 类实现
- [ ] 注册多个监听器测试
- [ ] 事件路由到正确监听器测试
...
- [ ] Triggered 事件定义位于 `src/domain/events/trigger_events.py`
- [ ] Routed 事件定义位于 `src/domain/events/route_events.py`
- [ ] Executed 事件定义位于 `src/domain/events/execute_events.py`
- [ ] Pydantic 模型验证通过

#### API 契约 (API Contract)
- [ ] App 入口 API：`app.run()` / `app.shutdown()`
- [ ] Container API：`register_singleton()` / `register_factory()` / `resolve()`
- [ ] EventListenerRegistry API：`register_listener()` / `get_handler()`
- [ ] 契约测试通过
...
- [ ] 功能测试文件：`tests/acceptance/test_acceptance_application_startup_and_integration.feature`
...
|---------|------|----------|----------|-----------|
| **TDD 单元测试** | AppConfig | 配置加载验证 | `test_app_config.py` | Task 1 |
...
| **TDD 验收测试** | Gherkin 场景 | 业务价值验收 | `test_acceptance_application_startup_and_integration.feature` | Task 0 |
...
| AC-3 | 事件监听器注册表 | Task 2 | Subtask 2.4-2.6 | `test_listener_registry.py` |
| AC-4 | 事件发布路径 | Task 3 | Subtask 3.1-3.3 | `test_event_publish.py` |
...
- [ ] Subtask 0.3: 创建 Gherkin 验收测试 `tests/acceptance/test_acceptance_application_startup_and_integration.feature`
...
#### TDD 循环 [B]：Consumer 事件消费

| 阶段 | 动作 |
|------|------|
| 🔴 红 | 编写 `test_event_consumption.py`（验证消费流程） |
| 🟢 绿 | 实现 Consumer 注册与启动 |
| 🔄 重构 | 优化 Consumer 集成代码 |

- [ ] Subtask 3.4: 🔴 红 — 编写 Consumer 集成失败测试
- [ ] Subtask 3.5: 🟢 绿 — 实现 Consumer 注册与启动
- [ ] Subtask 3.6: 🔄 重构 — 优化 Consumer 集成代码
...
- **架构模式:** 六边形架构（应用层可依赖基础设施层，领域层零依赖）
- **设计约束:** 依赖方向规则（应用层 → 基础设施层）
- **技术栈:** Python 3.11+, aio-pika, asyncio, redis, sqlalchemy
...
│           └── auto_execute_completed_listener.py
├── tests/
│   ├── unit/application/
│   │   ├── test_app_config.py     # AppConfig 单元测试
│   │   ├── test_app.py            # App 单元测试
│   │   ├── test_di_container.py     # Container 单元测试
│   │   └── test_listener_registry.py  # ListenerRegistry 单元测试
│   ├── integration/
│   │   ├── test_app_lifecycle.py   # 生命周期集成测试
│   │   └── test_event_consumption.py  # 事件消费集成测试
│   └── acceptance/
│       ├── test_acceptance_application_startup_and_integration.feature   # Gherkin 场景
...
- DI 容器设计要支持单例和工厂两种模式
- BDD 步骤函数使用 `event_loop.run_until_complete()` 运行 async

**应用到本故事/Applied to This Story:**
- [ ] 应用层 DI 容器不污染领域层
- [ ] 使用 UUID 前缀隔离测试数据
- [ ] Container 支持 register_singleton 和 register_factory
- [ ] BDD 步骤函数正确使用 event_loop
...
- `tests/unit/application/test_app_config.py` - AppConfig 单元测试
...
- `tests/acceptance/test_acceptance_application_startup_and_integration.feature` - Gherkin 场景
