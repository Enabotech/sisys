...
- [x] 功能测试文件：`tests/acceptance/test_acceptance_minio_object_layer.feature`
- [x] 场景覆盖：
...
| **TDD 验收测试** | Gherkin 场景 | 业务价值验收（端到端对象存储操作） | `test_acceptance_minio_object_layer.feature` | Task 0 |
...
|----|-------------|-----------|-------------|----------|
| AC-1 | MinIO 客户端适配器就绪 | Task 0 | SDD 规范定义（数据模型、MinIOConfig、仓储接口、验收测试） | `test_acceptance_minio_object_layer.feature` |
...
- [x] Subtask: 创建 Gherkin 验收测试 `tests/acceptance/test_acceptance_minio_object_layer.feature`
...
│   └── acceptance/
│       └── test_acceptance_minio_object_layer.feature          # Gherkin 验收测试
└── docs/
    └── infrastructure/
...
- `tests/acceptance/test_acceptance_minio_object_layer.feature` — Gherkin 验收测试（8 场景）
