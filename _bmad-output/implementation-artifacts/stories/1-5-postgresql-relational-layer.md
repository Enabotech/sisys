...
- [ ] 功能测试文件：`tests/acceptance/test_acceptance_postgresql_relational_layer.feature`
- [ ] 覆盖场景:
  - 数据库连接与健康检查
  - Alembic 迁移执行与回滚
  - Outbox 事件保存与轮询
...
- [x] Subtask: 编写 Gherkin 验收测试 `tests/acceptance/test_acceptance_postgresql_relational_layer.feature`
...
│   └── acceptance/
│       └── test_acceptance_postgresql_relational_layer.feature
└── docs/
    └── infrastructure/
...
| `tests/acceptance/test_acceptance_postgresql_relational_layer.feature` | Gherkin 验收测试 | ✅ 已创建 |
...
| 文件 | 修改 |
|------|------|
| `tests/acceptance/test_acceptance_postgresql_relational_layer.py` | 添加 `ensure_alembic_migration` fixture；添加 "Alembic 升级迁移执行成功" 场景；修复 `pg_config` scope 不匹配 |
