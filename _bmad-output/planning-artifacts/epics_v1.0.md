...
| Story 0.2 | **CI/CD 流水线** | 自动化构建、测试和部署 | 依赖 Story 0.1 | **P0-0（Iteration 0）** |
...
| Story 1.14 | **自主调用循环实现** | 实现 trigger→route→execute 自主调用 | 依赖 Story 1.2/1.3 | **P0-14（or.md 系统公理一）** |
| Story 1.15 | **外部化记忆实现** | 实现 LLM 上下文=缓存、磁盘记忆=真相源 | 依赖 Story 1.4-1.8 | **P0-15（or.md 系统公理二）** |
...
- **Story 1.14**：覆盖 or.md 系统公理一（自主调用：trigger→route→execute）
- **Story 1.15**：覆盖 or.md 系统公理二（外部化记忆：LLM 上下文=缓存，磁盘记忆=真相源）
...
As a **系统架构师**,
I want **实现 trigger→route→execute 自主调用循环**,
So that **系统可以基于事件自主执行任务**。

**Acceptance Criteria:**

**Given** 领域事件触发（或周期性心跳事件）
**When** 自主调用循环启动
**Then** 基于 session_id 哈希或语义路由至目标 Agent/工具
**And** 在会话命名空间中执行，状态快照序列化至 Redis Hash（支持主从复制与故障转移，TTL 24h-30d）

### Story 1.15: 外部化记忆实现

As a **系统架构师**,
I want **实现 LLM 上下文=缓存、磁盘记忆=真相源的记忆分离**,
So that **防止上下文爆炸并支持长期记忆**。
...
**And** 会话状态与推理轨迹持久化存储至战略档案库（五层存储：Redis/PostgreSQL/Qdrant/MinIO/Neo4j）
...
| **Epic 1: 企业级架构基础与合规** | 系统稳定性与性能基础<br/>安全与合规基础<br/>or.md 系统公理 | - | - | - | 组内依赖 | ✅ 是 | **15** |
...
| **Epic 7: 多触点用户界面与 API 集成** | 多触点操作与监控能力<br/>NFR 覆盖 | Story 1.1/1.9/1.12 | - | - | 并行依赖 | ✅ 是 | **6** |
| **Epic 8: 用户权限管理与审计合规** | 安全与合规 | Story 1.5/1.7/1.9/1.10/1.12 | - | - | 顺序依赖（安全测试） | ✅ 是 | 6 |

**总计 Story 数：79 个**
- Iteration 0: 3 个（Story 0.1-0.3）
- Epic 1: 15 个（Story 1.1-1.15，新增 Story 1.13-1.15）
- Epic 2: 8 个
- Epic 3: 12 个
- Epic 4: 5 个
- Epic 5: 6 个
- Epic 6: 8 个
- Epic 7: 6 个（新增 Story 7.5-7.6）
- Epic 8: 6 个
...
7. **or.md 系统公理覆盖** - Story 1.14（自主调用）、Story 1.15（外部化记忆）
...
Epic 0（Iteration 0）
    ...
    └──→ Epic 1（企业级架构基础与合规）
            ...
            ├──→ Story 1.14（自主调用循环，or.md 系统公理一）
            └──→ Story 1.15（外部化记忆，or.md 系统公理二）
            │
...
| 建议 | 状态 | 实施说明 |
|------|------|---------|
| **第 5 条：增加 Iteration 0** | ✅ 已完成 | Story 0.1（开发环境）、Story 0.2（CI/CD）、Story 0.3（测试框架） |
| **第 6 条：增加测试 Story** | ⏸️ 待实施 | 建议每个 Epic 增加 Story X.9（集成测试）、Story X.10（E2E 测试） |
| **第 8 条：增加 or.md 追溯** | ✅ 已完成 | Story 1.14（自主调用循环）、Story 1.15（外部化记忆） |

---

**文档状态：** 本文档已完成 **Step 02（Epic 设计）**，P0 关键改进建议（第 2/3 条）已实施，P1 关键改进建议（第 5/8 条）已实施，所有 Epic 价值组和依赖关系验证已完成

**总计 Story 数：79 个**（原 71 个 + 新增 8 个：Iteration 0 的 3 个 + NFR 的 3 个 + or.md 的 2 个）
