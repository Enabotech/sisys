...
**V1 阶段升级：**
- 🔜 高级限流（多维度：IP/用户/API Key）
- 🔜 高级路由（基于语义/负载/地理位置）
- 🔜 高级安全控制（提示注入检测/ShieldCortex 集成）
- 🔜 监控仪表板（实时 API 指标/告警）
- 🔜 灰度发布（基于权重/用户标签）
...
**性能要求：**
- 溯源树加载延迟：P95<500ms
- 坐标跳转延迟：P95<300ms（UX 核心体验指标）

**MVP 阶段说明：**
- ✅ 溯源树展示（结论层→分析层→数据层→原始层）
- ✅ 逐层展开/收起
- ✅ Bounding Box 级坐标跳转（P95<300ms）
- ✅ 置信度评分显示（高/中/低）
- ✅ 溯源树导出（JSON 格式）
...
- `confidence_delta`：置信度变化值（修正操作时记录）
- `impact_scope`：影响范围（受影响的后续任务数量）
- `worm_storage_ref`：WORM 存储引用（V1 升级后指向 MinIO WORM Bucket）

**And** 审计日志存储至 PostgreSQL（MVP 阶段）
**And** V1 升级后同步归档至 MinIO WORM 存储（7 年不可篡改）
...
**And** 边界处理：得分位于边界±0.03 范围内时，标记"低置信度判定"并建议人工复核
...
- **入侵防范**：集成 ShieldCortex 提示注入检测器（正则匹配已知攻击模式+ML 分类器检测未知变体，准确率≥95%，误报率<5%），防火墙规则
...
### Story 9.1: UDMR 框架 - L1 合规性过滤功能

As a **安全合规官**,
I want **执行 UDMR L1 合规性过滤**,
So that **确保所有 LLM 路由决策符合数据安全法规和模型白名单要求**。
...
**When** 执行 L1 合规性检查
**Then** 系统执行以下检查流程：
- **敏感数据检查**：检测 PII（个人可识别信息）和商业秘密（财务数据/合同金额/客户名单）
- **数据驻留限制**：检查数据境内/跨境要求，境内数据强制本地路由
- **模型白名单校验**：验证目标模型在允许的模型列表中

**And** 敏感数据强制本地路由
**And** 数据境内存储要求强制本地路由
**And** 模型白名单定期更新（每周）
**And** L1 合规性过滤延迟 P95<10ms
...
### Story 9.2: UDMR 框架 - L2 任务复杂度评估功能

As a **路由策略工程师**,
I want **执行 UDMR L2 任务复杂度评估**,
So that **基于四因子评分选择最优模型路由**。
...
**When** 执行 L2 任务复杂度评估
**Then** 系统计算四因子评分：
- **语义匹配度 (35%)**：计算任务与模型能力的语义相似度（bge-m3 嵌入）
- **历史成功率 (30%)**：查询该模型在相似任务上的历史成功率
- **成本效率 (20%)**：计算单位 Token 成本效益比
- **任务复杂度 (15%)**：评估任务复杂度（简单/中等/复杂）

**And** 综合评分 = 语义匹配×0.35 + 历史成功率×0.30 + 成本效率×0.20 + 任务复杂度×0.15
**And** 输出各候选模型综合评分排序
**And** L2 任务复杂度评估延迟 P95<30ms
...
### Story 9.3: UDMR 框架 - L3 路由决策执行功能

As a **路由决策引擎**,
I want **执行 UDMR L3 路由决策执行**,
So that **基于 L2 评分做出最终路由决策并记录完整日志**。

**Acceptance Criteria:**

**Given** 系统完成 L2 任务复杂度评估
**When** 执行 L3 路由决策
**Then** 系统执行决策逻辑：
- **云模型优势阈值**：0.15（云端评分 - 本地评分 > 0.15 时选择云端）
- **本地质量阈值**：0.70（本地评分≥0.70 时优先本地）
- **决策输出**：选定模型 + 预估成本 + 路由延迟

**And** 路由决策日志记录完整（任务 ID、时间戳、L1 结果、L2 各因子评分、最终评分、选定路由、成本、延迟、`worm_storage_ref` WORM 归档引用）
**And** 本地路由占比≥60%（MVP 目标）/ ≥80%（V1 目标）
**And** 路由决策总延迟 P95<50ms

**质量要求：**
- 路由决策准确率：≥85%（MVP 目标）/ ≥95%（V1）
- 本地路由占比：≥60%（MVP）/ ≥80%（V1）
- 成本节省：≥30%（MVP）/ ≥50%（V1）
- 日志完整性：100%
...
As a **运维经理**,
I want **提供路由策略可视化仪表盘**,
So that **直观了解路由优化效果、本地/云端比例和成本趋势**。

**Acceptance Criteria:**

**Given** 运维人员需要监控路由策略效果
**When** 访问路由策略仪表盘
**Then** 系统展示以下可视化指标：
- **本地/云端路由比例图**：饼图展示本地路由占比（目标≥80%）
- **成本趋势图**：折线图展示每日/周/月成本趋势
- **路由决策延迟分布**：直方图展示延迟 P50/P95/P99
- **模型使用分布**：柱状图展示各模型使用频率
- **成本节省统计**：Token 成本节省对比（使用 UDMR vs 纯云端）

**And** 指标实时更新（每秒刷新）
**And** 支持时间范围选择（今日/本周/本月/自定义）
**And** 支持仪表盘导出（PNG/PDF）
...
**When** 监控成本消耗
**Then** 系统执行三级熔断机制：
- **任务级熔断**：单次 5K tokens（简单任务）或 50K tokens（复杂任务）超预算 50% 时熔断，通知用户
- **会话级熔断**：日配额 100 万 tokens 超预算 80% 时熔断，通知管理员
- **系统级熔断**：系统总成本超预算 90% 时熔断，通知所有用户；连续 3 次失败触发系统级熔断

**And** 成本预测基于历史相似任务
**And** 偏差超阈值触发分级预警（黄色/橙色/红色）
**And** 成本数据记录至审计日志
**And** 熔断后采用指数退避策略恢复（1s/2s/4s，最大 3 次）
...
**And** 缓存监控指标（命中率统计、Token 节省量、缓存失效分析报表）
...
- **季节性调整**：支持按周/月/季度对比基线，应用季节性调整因子消除周期性波动影响
...
### Story 9.9: OpenTelemetry Trace 功能
...
| Story | 标题 | 覆盖 FR | 状态 |
|-------|------|--------|------|
| 9.1 | UDMR 框架 - L1 合规性过滤功能 | CP-01 | ✅ 完成 |
| 9.2 | UDMR 框架 - L2 任务复杂度评估功能 | CP-02 | ✅ 完成 |
| 9.3 | UDMR 框架 - L3 路由决策执行功能 | CP-03 | ✅ 完成 |
| 9.4 | 路由策略可视化功能 | CP-03（增强） | ✅ 完成 |
| 9.5 | 三级成本熔断功能 | CP-04, CP-05 | ✅ 完成 |
| 9.6 | 语义缓存功能 | CP-06, CP-07 | ✅ 完成 |
| 9.7 | CUSUM 性能漂移检测功能 | CP-08 | ✅ 完成 |
| 9.8 | 健康度仪表盘功能 | CP-09 | ✅ 完成 |
| 9.9 | OpenTelemetry Trace 功能 | CP-10 | ✅ 完成 |

**FR 覆盖验证：**
- CP-01: UDMR 三层决策 → Story 9.1 + Story 9.2 + Story 9.3
- CP-02: 四因子评分路由 → Story 9.2
- CP-03: 路由决策日志 → Story 9.3 + Story 9.4（可视化）
- CP-04: 三级成本熔断 → Story 9.5
- CP-05: 成本预测和偏差预警 → Story 9.5
- CP-06: 语义缓存 → Story 9.6
- CP-07: 缓存失效管理 → Story 9.6
- CP-08: 性能漂移检测 → Story 9.7
- CP-09: 健康度仪表盘 → Story 9.8
- CP-10: OpenTelemetry Trace → Story 9.9

**改进说明：**
- ✅ 问题 1 修复：Story 9.1 已拆分为 Story 9.1/9.2/9.3（L1/L2/L3 三层）
- ✅ 问题 2 修复：Story 9.6 已增加缓存监控指标
- ✅ 问题 3 修复：Story 9.4 已增加路由策略可视化
...
**And** 存储完整性 100%
**And** 支持历史版本对比
**And** V1 升级支持分支管理（主线/分支差异对比、分支合并/放弃）
...
- 存储延迟：P95<500ms
- 历史版本对比延迟：P95<2 秒
- TB 级战略档案库支持，检索延迟 P95<1s（NFR-SCALE-02）

**业务规则：**
- SP/BP 永久存储（不删除）
- 支持按时间范围查询
- 支持版本差异对比
- 多租户隔离：Schema per Tenant + Row-Level Security（NFR-SCALE-04）

**MVP 阶段说明：**
- ✅ 关键假设变量存储（市场增长率/竞争格局/技术趋势）
- ✅ 决策依据存储（输入数据/工具调用/Agent 建议/置信度评分）
- ✅ 实际执行偏差存储
- ✅ L2 关系存储（PostgreSQL）+ L4 对象存储（MinIO）
- ✅ 历史版本对比基础功能
- ✅ 多租户隔离（Schema per Tenant + RLS）

**V1 阶段升级：**
- 🔜 L3 向量存储（Qdrant）+ L5 图存储（Neo4j）
- 🔜 完整五层存储协同
- 🔜 分支管理（主线/分支差异对比、分支合并/放弃）
- 🔜 TB 级存储优化（检索延迟 P95<1s）
...
- **成本预算校验**：检查会话成本是否超预算，失败 3 次触发指数退避（1s/2s/4s，最大 3 次）
- **性能漂移检查**：CUSUM 算法检测连续性能下降（滑动窗口 7 天，检测准确率≥85%）

**And** 心跳日志记录
**And** 支持心跳频率配置（小时/天/周）
**And** 支持心跳任务自定义
**And** 连续失败 3 次后暂停心跳，通知管理员

**质量要求：**
- 心跳执行完整率：100%
- 待办事项提醒及时率：100%
- 偏差预警准确率：≥90%
- CUSUM 检测准确率：≥85%（NFR-REL-05）

**性能要求：**
- 心跳唤醒延迟：P95<100ms
- 待办事项检查延迟：P95<500ms
- 偏差预警延迟：P95<1 秒
- CUSUM 检测延迟：P95<500ms
...
**Then** 系统执行以下流程：
- **偏差计算**：规划目标 - 实际执行 = 偏差值
- **偏差类型识别**：财务偏差（营收/利润/现金流）、运营偏差（效率/质量/成本）、市场偏差（份额/增长/客户）
- **偏差判定**：偏差超阈值 10% 自动触发预警
- **预警发布**：发布战略偏差预警事件（邮件/短信/钉钉）
- **偏差分级**：一般偏差（10-20%）、严重偏差（20-50%）、紧急偏差（>50%）
