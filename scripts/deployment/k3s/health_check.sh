...
if [ "$FAILED" -eq 0 ]; then
    ...
    if [ "$SYSTEM_PODS" -ne 0 ]; then
        ...
        FAILED=2
        FAILURE_REASON="系统 Pod 异常 ($SYSTEM_PODS 个未运行)"
    else
...
if [ "$FAILED" -eq 0 ]; then
    ...
    if [ "$STORAGE_CLASS" = "rancher.io/local-path" ]; then
        ...
        if [ "$STORAGE_CLASS" = "rancher.io/local-path" ]; then
            ...
            FAILED=3
            FAILURE_REASON="存储配置异常 (当前：$STORAGE_CLASS)"
        fi
    fi
...
if kubectl get namespace traefik &>/dev/null; then
    ...
    if [ "$TRAEFIKS" -ne 0 ]; then
        ...
        TRAEFIK_FAILED=1
        echo "⚠️ 警告：Traefik 异常，但不影响主检查流程"
    else
...
echo ""
echo "集群状态总结："
echo "  - K3S 集群：$([ "$NODE_STATUS" = "True" ] && echo "Ready" || echo "Not Ready")"
if [ "$FAILED" -eq 0 ]; then
    echo "  - 系统 Pod：全部 Running"
...
if kubectl get namespace traefik &>/dev/null; then
    echo "  - Traefik 反向代理：已安装 $([ "$TRAEFIK_FAILED" -eq 0 ] && echo "(正常)" || echo "(异常)")"
else
...
if [ "$FAILED" -ne 0 ]; then
    ...
    echo "   1=节点未就绪，2=系统 Pod 异常，3=存储配置异常"
    echo "   原因：$FAILURE_REASON"
    exit $FAILED
fi

# Traefik 异常但不影响主流程
if [ "$TRAEFIK_FAILED" -ne 0 ]; then
    echo "⚠️ 健康检查通过但 Traefik 异常（退出码：5）"
    echo "   建议检查 Traefik 配置和日志"
    exit 5
fi
