...
class AsyncpgPostgreSQLToolExecutionRepository(ToolExecutionRepositoryPort):
    """PostgreSQL ToolExecution 仓储(asyncpg 直连,Story 4.3 后续技术债清理前的过渡实现)
    ...
    当前状态：保留作为测试 fixture 使用(避免 SQLAlchemy ContextVar 在 pytest-xdist
    多进程下不稳定)。生产路径通过 composition_root 使用 SQLAlchemy ORM 实现。
    """
