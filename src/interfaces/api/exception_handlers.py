...
EXCEPTION_HTTP_MAP: dict[type[DomainError], int] = {
    ...
    # Story 4.4 新增 5 个沙箱异常（EXCEPTION_315-319）HTTP 映射
    SandboxImagePullError: status.HTTP_502_BAD_GATEWAY,  # 315 — 镜像拉取失败（上游不可达）
    SandboxTimeoutError: status.HTTP_504_GATEWAY_TIMEOUT,  # 316 — 执行超时
    SandboxResourceLimitExceededError: status.HTTP_502_BAD_GATEWAY,  # 317 — cgroups 资源超限
    SandboxQuotaExceededError: status.HTTP_503_SERVICE_UNAVAILABLE,  # 318 — 并发容器数超 MAX_CONCURRENT_CONTAINERS
    SandboxConfigurationError: status.HTTP_502_BAD_GATEWAY,  # 319 — 运行时配置错误（与父类 SandboxError 一致）
    # OCR 异常
