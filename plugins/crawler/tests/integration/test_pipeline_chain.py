...
class TestPipelineChain:
    ...
    def test_pdf_pipeline_chain(self) -> None:
        ...
        item = download_pipeline.process_item(item, self.spider)
        assert item["file_size"] > 0

        # Pipeline 2: 格式检测
        format_pipeline = FormatDetectionPipeline()
        format_pipeline.open_spider(self.spider)
        item = format_pipeline.process_item(item, self.spider)
        assert item["detected_format"] == "pdf"

        # Pipeline 3: 元数据提取
        metadata_pipeline = MetadataPipeline()
        metadata_pipeline.open_spider(self.spider)
        item = metadata_pipeline.process_item(item, self.spider)
        assert "metadata_title" in item
        assert "metadata_author" in item

        # Pipeline 4: 智能命名
        naming_pipeline = SmartNamingPipeline()
        naming_pipeline.open_spider(self.spider)
        item = naming_pipeline.process_item(item, self.spider)
    ...
    def test_empty_file_dropped(self) -> None:
        ...
        with pytest.raises(DropItem):
            download_pipeline.process_item(item, self.spider)
