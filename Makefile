...
db-downgrade:
	@echo "⏮️  回滚数据库..."
	$(POETRY) run alembic -c deploy/alembic/alembic.ini downgrade -1

db-upgrade:
	@echo "⏭️  升级数据库到指定版本..."
	$(POETRY) run alembic -c deploy/alembic/alembic.ini upgrade $(revision)

db-head:
	@echo "📍 查看当前数据库版本..."
	$(POETRY) run alembic -c deploy/alembic/alembic.ini heads

db-revision:
	@echo "📝 创建新的数据库迁移..."
	$(POETRY) run alembic -c deploy/alembic/alembic.ini revision -m "$(message)"
