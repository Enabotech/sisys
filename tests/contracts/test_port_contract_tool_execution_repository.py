"""Story 4.1a: ToolExecutionRepository 端口契约测试（11 维度）
...
class _DummyResolver:
    ...
    def resolve(self, name: str) -> Any:
        from src.infrastructure.storage.inmemory.tool_execution_repository import (
...
class TestToolExecutionRepositoryPortContract:
    """ToolExecutionRepositoryPort 端口契约测试 11 维度"""
    ...
    def test_dimension_4_port_interface_type(self) -> None:
        ...
        spec = self._spec()
        assert spec is not None
        assert spec.interface is ToolExecutionRepositoryPort
    ...
    def test_dimension_9_impl_factory_produces_port_instance(self) -> None:
        ...
        assert isinstance(instance, ToolExecutionRepositoryPort), f"{self.IMPL_CLS_NAME} 未实现 ToolExecutionRepositoryPort"
