"""Story 3.7 检索相关性评估端口契约测试

验证 relevance_evaluation_service 端口的注册、解析和实现。
...
class TestRelevanceEvaluationPortContract:
    """relevance_evaluation_service 端口契约"""

    PORT_NAME = "relevance_evaluation_service"
    IMPL_CLS_NAME = "RelevanceEvaluationService"
    REQUIRED_METHODS = ["evaluate", "quick_rule_check"]
    ...
    def test_port_is_registered(self, registry: PortRegistry) -> None:
        ...
        assert spec.interface is RelevanceEvaluationPort
...
class TestRelevanceEvaluationPortInterface:
    """RelevanceEvaluationPort 领域端口接口契约"""

    def test_port_protocol_exists(self) -> None:
        """RelevanceEvaluationPort 协议应存在于 domain/ports"""
        from src.domain.ports.relevance_evaluation import RelevanceEvaluationPort
...
class TestRelevanceEvaluationPortResolver:
    """RelevanceEvaluationService 端口解析验证"""

    def test_resolve_relevance_evaluation_service(self, resolver) -> None:
        """验证 Resolver 可解析 relevance_evaluation_service"""
        resolved = resolver.resolve("relevance_evaluation_service")
        assert hasattr(resolved, "evaluate")
        assert hasattr(resolved, "quick_rule_check")
