"""LLMClientPort 端口契约测试

验证 llm_client 端口的注册、实现和元数据。
...
class TestLLMClientPortContract:
    """LLMClientPort 端口契约"""

    PORT_NAME = "llm_client"
    IMPL_CLS_NAME = "LitellmLLMClient"
    REQUIRED_METHODS = ["generate", "structured_generate", "close"]
    ...
    def test_port_is_registered(self, registry: PortRegistry) -> None:
        ...
        assert spec.interface is LLMClientPort
