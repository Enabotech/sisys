...
功能: Story 1.3 - 双通道事件总线实现
  ...
  场景: AC-5 - 事件处理监控和可观测性
    ...
    当 事件处理失败
    那么 events_failed_total 计数器应该递增
    那么 当 EVENT_BUS_OTEL_TRACE_ENABLED=true 时应该创建 OpenTelemetry span

  场景: AC-6 - 架构约束验证
    当 我运行架构约束验证测试
    那么 领域层不应该导入 OutboxEntity
    那么 Redis/RabbitMQ 客户端导入应该只在基础设施层
    那么 Ruff 检查应该通过
    那么 MyPy 类型检查应该通过
  ...
  场景: AC-7 - 事件处理重试机制（指数退避 + 抖动）
    当 事件处理失败并触发重试
    那么 重试延迟应该遵循指数退避
    那么 jitter 应该在 0.5 和 1.5 之间
    那么 超过最大重试次数后事件应该进入死信队列
