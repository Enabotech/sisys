...
功能: Story 1.11 - 数据主权隔离
  ...
  场景: AC-1 - 检测 PII 数据
    假如 待检测内容包含身份证号
    并且 内容设置为张三的身份证号110101199001011234
    当 执行敏感数据检测
    那么 系统识别出 PII 类型数据
    并且 检测置信度大于 0.8
  ...
  场景: AC-1 - 检测金融数据
    假如 待检测内容包含银行账号
    并且 内容设置为银行账号6222021234567890123
    当 执行敏感数据检测
