...
功能: Redis 缓存层
  ...
  场景: 会话状态保存与恢复
    假如 调用 SessionStorage.save 保存会话
    当 调用 SessionStorage.load 加载会话
    那么 返回的会话状态与保存的一致

  场景: 会话状态删除
    假如 会话状态已保存
    当 调用 SessionStorage.delete 删除会话
    并且 调用 SessionStorage.load 加载会话
    那么 返回 None

  场景: 会话状态过期
    假如 会话状态已保存并设置 TTL 为 1 秒
  ...
  场景: 语义缓存命中率统计
    ...
    当 执行多次缓存命中和未命中
    那么 查询 EventMetricsCollector.hit_rate
  ...
  场景: 公共黑板多 Agent 并发写入
    假如 Agent 发布消息
    当 调用 PublicBlackboard.get 读取会话
    那么 返回所有 Agent 发布的消息
    并且 消息按时间戳排序

  场景: 公共黑板版本号递增
    假如 Agent 向会话发布第 1 条消息
    当 Agent 再次向会话发布消息
    那么 返回的版本号递增
  ...
  场景: Redis 键批量清理
    假如 命名空间下有多个键
    当 调用 RedisCleanup.cleanup_namespace
    那么 返回删除的键数量
