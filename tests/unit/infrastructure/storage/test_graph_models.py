...
class TestGraphNode:
    ...
    def test_no_labels_raises_error(self):
        """测试无标签应抛出 ValueError"""
        with pytest.raises(ValidationError, match="Node must have at least one label"):
            GraphNode(
                id="entity-001",
                labels=[],
                properties={"business_domain": "test", "entity_type": "Entity", "content_hash": "hash"},
    ...
    def test_missing_required_properties_raises_error(self):
        ...
        with pytest.raises(ValidationError, match="Node properties must include"):
            GraphNode(
                id="entity-001",
                labels=["sisys:Entity"],
...
class TestGraphRelationship:
    ...
    def test_empty_start_node_id_raises_error(self):
        ...
        with pytest.raises(ValidationError, match="start_node_id must be a non-empty string"):
            GraphRelationship(
                start_node_id="",
                end_node_id="entity-002",
                relationship_type=RelationshipType.MENTIONS,
    ...
    def test_empty_end_node_id_raises_error(self):
        ...
        with pytest.raises(ValidationError, match="end_node_id must be a non-empty string"):
            GraphRelationship(
                start_node_id="entity-001",
                end_node_id="",
                relationship_type=RelationshipType.MENTIONS,
    ...
    def test_empty_relationship_type_raises_error(self):
        ...
        with pytest.raises(ValidationError, match="relationship_type must be specified"):
            GraphRelationship(
                start_node_id="entity-001",
                end_node_id="entity-002",
                relationship_type="",
