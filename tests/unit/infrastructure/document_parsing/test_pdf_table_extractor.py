...
class TestPdfTableExtractorStandard:
    """标准 PDF 表格检测测试"""

    @patch("src.infrastructure.document_parsing.pdf_table_extractor.pdfplumber")
...
class TestPdfTableExtractorDegradation:
    """降级策略测试"""
    ...
    def test_non_pdf_mime_returns_empty(self) -> None:
        """非 PDF MIME 类型直接返回空"""
        extractor = PdfTableExtractor()
        result = extractor.extract("/tmp/test.txt", "text/plain", [])
        assert result == []


class TestPdfTableExtractorProtocol:
    """Protocol 合规性测试"""

    def test_implements_table_extractor_port(self) -> None:
        """验证 PdfTableExtractor 满足 TableExtractorPort Protocol"""
...
class TestPdfTableExtractorReturnValues:
    """返回值结构测试"""
