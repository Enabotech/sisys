...
使用 mock httpx.AsyncClient 避免真实网络调用。
EmbeddingAPIClient 方法签名为 async def，测试使用 @pytest.mark.asyncio。
...
class TestEmbeddingAPIClientInit:
    ...
    @pytest.mark.asyncio
    async def test_api_url_empty_raises(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_init_with_api_url(self, api_config: EmbeddingConfig) -> None:
        """api_url 非空时正常初始化"""
        client = EmbeddingAPIClient(api_config)
        assert client is not None

    @pytest.mark.asyncio
    async def test_dimension_property(self, api_config: EmbeddingConfig) -> None:
        """dimension 属性返回 1024"""
        client = EmbeddingAPIClient(api_config)
...
class TestEmbeddingAPIClientEmbedQuery:
    ...
    @pytest.mark.asyncio
    async def test_embed_query_returns_1024_dim(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_embed_query_sends_correct_payload(self, api_config: EmbeddingConfig) -> None:
...
class TestEmbeddingAPIClientEmbedDocuments:
    ...
    @pytest.mark.asyncio
    async def test_embed_documents_batch(self, api_config: EmbeddingConfig) -> None:
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_resp):
            client = EmbeddingAPIClient(api_config)
            result = await client.embed_documents(["文本一", "文本二", "文本三"])
    ...
    @pytest.mark.asyncio
    async def test_embed_documents_empty_list(self, api_config: EmbeddingConfig) -> None:
        """空列表不发送 HTTP 请求"""
    ...
    @pytest.mark.asyncio
    async def test_embed_documents_mixed_invalid_raises(self, api_config: EmbeddingConfig) -> None:
        """批量中包含空文本时抛出 ValueError"""
        with patch("httpx.AsyncClient.post") as mock_post:
            client = EmbeddingAPIClient(api_config)
            with pytest.raises(ValidationError, match="文本列表"):
                await client.embed_documents(["有效文本", "", "另一个有效文本"])
            mock_post.assert_not_called()

    @pytest.mark.asyncio
    async def test_embed_documents_whitespace_item_raises(self, api_config: EmbeddingConfig) -> None:
        """批量中包含纯空白项时抛出 ValueError"""
        with patch("httpx.AsyncClient.post") as mock_post:
            client = EmbeddingAPIClient(api_config)
            with pytest.raises(ValidationError, match="文本列表"):
                await client.embed_documents(["有效文本", "   "])
            mock_post.assert_not_called()
...
class TestEmbeddingAPIClientEmbedSparse:
    ...
    @pytest.mark.asyncio
    async def test_embed_sparse_returns_correct_format(self, api_config: EmbeddingConfig) -> None:
        """embed_sparse 返回 list[SparseEmbedding]"""
        mock_resp = _make_mock_response(_fake_sparse_response())

        with patch("httpx.AsyncClient.post", return_value=mock_resp):
            client = EmbeddingAPIClient(api_config)
            result = await client.embed_sparse(["测试文本"])
    ...
    @pytest.mark.asyncio
    async def test_embed_sparse_sends_return_sparse_true(self, api_config: EmbeddingConfig) -> None:
        """embed_sparse 发送 return_sparse=True"""
        mock_resp = _make_mock_response(_fake_sparse_response())

        with patch("httpx.AsyncClient.post", return_value=mock_resp) as mock_post:
            client = EmbeddingAPIClient(api_config)
            await client.embed_sparse(["测试文本"])

            call_kwargs = mock_post.call_args.kwargs
            assert call_kwargs["json"]["return_sparse"] is True

    @pytest.mark.asyncio
    async def test_embed_sparse_empty_response_returns_empty_list(self, api_config: EmbeddingConfig) -> None:
...
class TestEmbeddingAPIClientValidation:
    ...
    @pytest.mark.asyncio
    async def test_embed_query_empty_raises(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_embed_query_whitespace_raises(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_embed_sparse_empty_raises(self, api_config: EmbeddingConfig) -> None:
        """空文本 embed_sparse 抛出 ValueError"""
        with patch("httpx.AsyncClient.post") as mock_post:
            client = EmbeddingAPIClient(api_config)
            with pytest.raises(ValidationError, match="文本列表"):
                await client.embed_sparse([""])
            mock_post.assert_not_called()
...
class TestEmbeddingAPIClientErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_http_error_raises_embedding_api_error(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_timeout_raises_timeout_error(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_network_error_raises_network_error(self, api_config: EmbeddingConfig) -> None:
        """httpx.NetworkError 包装为 NetworkError (EXCEPTION_102)"""
        with patch("httpx.AsyncClient.post", side_effect=httpx.NetworkError("connection refused")):
            client = EmbeddingAPIClient(api_config)
            with pytest.raises(NetworkError, match="网络错误"):
                await client.embed_query("测试文本")

    @pytest.mark.asyncio
    async def test_invalid_json_raises_embedding_response_error(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_closed_client_raises_service_unavailable(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_close_idempotent(self, api_config: EmbeddingConfig) -> None:
        """重复调用 close() 不抛异常（幂等）"""
        client = EmbeddingAPIClient(api_config)
        await client.close()
        await client.close()  # 第二次调用不抛异常
        assert client._closed is True

    @pytest.mark.asyncio
    async def test_close_sets_closed_on_client_error(self, api_config: EmbeddingConfig) -> None:
        """httpx.AsyncClient.aclose() 抛异常时 _closed 仍为 True"""
        client = EmbeddingAPIClient(api_config)
        with patch.object(client._client, "aclose", side_effect=RuntimeError("socket error")):
            await client.close()
        assert client._closed is True
...
class TestEmbeddingAPIClientResponseValidation:
    ...
    @pytest.mark.asyncio
    async def test_missing_dense_key_raises_response_error(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_dense_not_list_raises_response_error(self, api_config: EmbeddingConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_dense_count_mismatch_raises_response_error(self, api_config: EmbeddingConfig) -> None:
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_resp):
            ...
            with pytest.raises(EmbeddingResponseError, match="向量数.*不匹配"):
                await client.embed_documents(["文本一", "文本二", "文本三"])

    @pytest.mark.asyncio
    async def test_response_not_dict_raises_response_error(self, api_config: EmbeddingConfig) -> None:
...
class TestEmbeddingExceptionCodes:
    ...
    @pytest.mark.asyncio
    async def test_embedding_api_error_code(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_embedding_response_error_code(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_embedding_model_error_code(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_timeout_error_code(self) -> None:
        """TimeoutError.code == EXCEPTION_302"""
        assert TimeoutError.code == "EXCEPTION_302"

    @pytest.mark.asyncio
    async def test_network_error_code(self) -> None:
