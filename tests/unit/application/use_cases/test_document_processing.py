...
class TestDocumentProcessingUseCase:
    ...
    @pytest.fixture
    def mock_outbox_repo(self) -> mock.AsyncMock:
        """创建 mock OutboxRepository"""
        return mock.AsyncMock(spec=OutboxRepository)

    @pytest.fixture
    def use_case(self, mock_outbox_repo: mock.AsyncMock) -> DocumentProcessingUseCase:
    ...
    async def test_process_document_success(
        ...
        """验证成功处理文档"""
        result = await use_case.process_document("doc-123")
    ...
    async def test_process_document_with_metadata(
        ...
        result = await use_case.process_document("doc-456", metadata=metadata)
    ...
    async def test_process_document_publishes_event(
        ...
        """验证处理文档时发布事件"""
        await use_case.process_document("doc-789")
    ...
    async def test_process_document_event_payload(
        ...
        """验证事件 payload 包含正确信息"""
        await use_case.process_document("doc-999")
