            ...
            for container in containers:
                if 'resources' in container:
                    ...
                    if 'requests' in resources:
                        ...
                        assert int(resources['requests']['nvidia.com/gpu']) >= 1, \
                            f"GPU request should be at least 1"
                ...
                if 'resources' in container:
                    ...
                    if 'limits' in resources:
                        assert 'nvidia.com/gpu' in resources['limits'], \
                            f"GPU limit not configured in {doc['metadata']['name']}"
                        assert int(resources['limits']['nvidia.com/gpu']) >= 1, \
                            f"GPU limit should be at least 1"
            ...
            if 'affinity' in spec:
                ...
                if 'nodeAffinity' in affinity:
                    ...
                    if 'requiredDuringSchedulingIgnoredDuringExecution' in node_affinity:
                        ...
                        assert gpu_label_found, \
                            f"GPU node affinity not properly configured in {doc['metadata']['name']}"
                ...
                for container in containers:
                    if 'resources' in container:
                        ...
                        if 'requests' in resources and 'nvidia.com/gpu' in resources['requests']:
                            assert gpu_toleration_found, \
                                f"GPU toleration not configured in {doc['metadata']['name']}"
...
class TestCIPipelineGPU:
    ...
    def test_unit_tests_gpu_support(self, ci_workflow):
        ...
        if 'unit-tests' in jobs:
            ...
            assert 'GPU_ENABLED' in unit_tests['if'], \
                "Unit tests should check GPU_ENABLED variable"
            
            # 检查 GPU Runner 标签
            if 'runs-on' in unit_tests:
                runs_on = unit_tests['runs-on']
                assert 'GPU_RUNNER_LABEL' in str(runs_on), \
                    "Unit tests should use GPU runner label"
    ...
    def test_integration_tests_gpu_support(self, ci_workflow):
        ...
        if 'integration-tests' in jobs:
            ...
            assert 'GPU_ENABLED' in integration_tests['if'], \
                "Integration tests should check GPU_ENABLED variable"
...
class TestDockerfileGPU:
    ...
    def test_pytorch_base_image(self):
        ...
        # 检查 CUDA 相关配置
        assert 'cuda' in content.lower() or 'CUDA' in content, \
            "Dockerfile should reference CUDA"
    ...
    def test_gpu_verification_step(self):
        ...
        # 检查 PyTorch CUDA 验证
        assert 'torch' in content.lower() or 'pytorch' in content.lower(), \
            "Dockerfile should include PyTorch for GPU support"
...
class TestGPUScripts:
    ...
    def test_import_pytorch_script_gpu_verification(self):
        ...
        assert '--gpus all' in content, \
            "Script should use --gpus all for GPU verification"
        assert 'torch.cuda.is_available' in content or 'CUDA' in content, \
            "Script should verify CUDA availability"
                ...
                for container in containers:
                    if 'env' in container:
                        ...
                        if 'GPU_ENABLED' in env_vars:
                            assert env_vars['GPU_ENABLED'] in ['true', 'True', '1'], \
...
class TestGPUDocumentation:
    ...
    def test_gpu_section_in_ci_cd_guide(self):
        ...
        assert len(found_keywords) >= 3, \
            "CI/CD guide should have comprehensive GPU documentation"
