...
class TestGiteaCredentialsConfig:
    ...
    def test_no_plaintext_token(self, gitea_credentials):
        ...
        # 验证密码不是明文（应该是环境变量占位符）
        assert not password.startswith('1f182aca3d38b66f7e49c034d98fb15bf02434b7'), \
            "CRITICAL-1: Gitea Token 仍然是明文！"
        assert '${GITEA_ADMIN_TOKEN}' in password or len(password) == 0, \
            "CRITICAL-1: 密码应该使用环境变量注入"
    ...
    def test_environment_variable_injection(self, gitea_credentials):
        ...
        # 验证使用环境变量占位符
        assert '${GITEA_ADMIN_TOKEN}' in password or '${' in password, \
            "应该使用环境变量注入 Token"
        assert '${GITEA_ADMIN_USERNAME}' in username or '${' in username, \
            "应该使用环境变量注入用户名"
...
class TestAdminSecretConfig:
    ...
    def test_no_plaintext_password(self, admin_secret):
        ...
        # 验证密码不是明文
        assert password != 'ArgoCD@2026Secure!', \
            "CRITICAL-2: Admin 密码仍然是明文！"
        assert '${ARGOCD_ADMIN_PASSWORD}' in password or len(password) == 0, \
            "CRITICAL-2: 密码应该使用环境变量注入"

    def test_environment_variable_injection(self, admin_secret):
        """验证使用环境变量注入"""
        password = admin_secret['stringData'].get('password', '')
        # 验证使用环境变量占位符
        assert '${ARGOCD_ADMIN_PASSWORD}' in password or '${' in password, \
            "应该使用环境变量注入密码"
        ...
        for doc in security_hardening:
            if doc:
                ...
                assert kind != 'PodSecurityPolicy', \
                    "CRITICAL-4: 仍然使用已废弃的 PodSecurityPolicy！"
...
class TestYamlSyntax:
    ...
    def test_all_yaml_files_valid_syntax(self, yaml_files):
        ...
        assert len(invalid_files) == 0, \
            f"以下 YAML 文件语法错误：{invalid_files}"
